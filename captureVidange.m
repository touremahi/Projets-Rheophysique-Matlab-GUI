function varargout = captureVidange(varargin)
% CAPTUREVIDANGE MATLAB code for captureVidange.fig
%      CAPTUREVIDANGE, by itself, creates a new CAPTUREVIDANGE or raises the existing
%      singleton*.
%
%      H = CAPTUREVIDANGE returns the handle to a new CAPTUREVIDANGE or the handle to
%      the existing singleton*.
%
%      CAPTUREVIDANGE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CAPTUREVIDANGE.M with the given input arguments.
%
%      CAPTUREVIDANGE('Property','Value',...) creates a new CAPTUREVIDANGE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before captureVidange_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to captureVidange_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help captureVidange

% Last Modified by GUIDE v2.5 07-Jul-2017 11:01:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @captureVidange_OpeningFcn, ...
                   'gui_OutputFcn',  @captureVidange_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before captureVidange is made visible.
function captureVidange_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to captureVidange (see VARARGIN)
...
% Choose default command line output for captureVidange
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes captureVidange wait for user response (see UIRESUME)
% uiwait(handles.fStat);


% --- Outputs from this function are returned to the command line.
function varargout = captureVidange_OutputFcn(hObject, eventdata, handles) 
