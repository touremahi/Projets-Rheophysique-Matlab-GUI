...
ordre=1;
myinterf=gcf;
global s posAct a b por cfgAll
%GPStastus
if cfgAll(5)
    descenteAct = 0;
end
...
%     t2=1:fenetre;
    ...
    %% Mesure Force
        ...
        if cfgAll(5)
            while (descenteAct==1)
                pas = str2double(get(findobj(myinterf, 'Tag','pasDescente'),'string'));
                vitPas = str2double(get(findobj(myinterf, 'Tag','vitDescente'),'string'));
                ti=pas/vitPas;
                vitMot=rpm2rated(mms2rpm(vitPas));
                set(findobj(myinterf, 'Tag','pState'),'String','Descente en cours...');
