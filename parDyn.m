function varargout = parDyn(varargin)
%PARDYN MATLAB code file for parDyn.fig
%      PARDYN, by itself, creates a new PARDYN or raises the existing
%      singleton*.
%
%      H = PARDYN returns the handle to a new PARDYN or the handle to
%      the existing singleton*.
%
%      PARDYN('Property','Value',...) creates a new PARDYN using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to parDyn_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      PARDYN('CALLBACK') and PARDYN('CALLBACK',hObject,...) call the
%      local function named CALLBACK in PARDYN.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help parDyn

% Last Modified by GUIDE v2.5 09-May-2017 16:18:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @parDyn_OpeningFcn, ...
                   'gui_OutputFcn',  @parDyn_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before parDyn is made visible.
function parDyn_OpeningFcn(hObject, eventdata, handles, varargin)
...
%         break;
	...
	set(findobj(gcf, 'Tag','nextF'),'enable','off');
%     set(findobj(gcf, 'Tag','backF'),'enable','off');
    set(findobj(gcf, 'Tag','pTot'),'enable','off');
    
    % Choose default command line output for parDyn
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes parDyn wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = parDyn_OutputFcn(hObject, eventdata, handles)
...
global posAct prLim s
    BtnName = questdlg('Assurer vous que vous avez vidé la cuve avant le le positionnement et enveler la porte métallique du fond de cuve, puis cliquez sur OK', 'Avertissement', 'OK','Annuler','OK');
if strcmp(BtnName,'OK')
            set(findobj(gcf, 'Tag','nextF'),'enable','off');
        %     set(findobj(gcf, 'Tag','backF'),'enable','off');
            set(findobj(gcf, 'Tag','pTot'),'enable','off');
            set(findobj(gcf, 'Tag','backF'),'enable','off');    
            set(findobj(gcf, 'Tag','validPos'),'enable','off');  
        set(findobj(gcf, 'Tag','pState'),'String','Positionnement en cours...');
        pPale=str2double(get(findobj(gcf, 'Tag','pDebut'),'string'));
...
%     pPale=pDynams(1);
            while abs(posAct-pPale)>0
                ...
                elseif dev>0
                        if (posAct-dev<prLim(1))||(posAct-dev>prLim(2))
                                errordlg('Déplacement Hors de la plage...','Erreur ...');
                                break;
                        end
...
set(findobj(gcf, 'Tag','backF'),'enable','on');
set(findobj(gcf, 'Tag','pTot'),'enable','on');
set(findobj(gcf, 'Tag','validPos'),'enable','on');    
