...
%     end
    ...
    if ~((0<vitPas)&&(vitPas<vLim))
            errordlg(['VitposAct=getPosition2(s,prLim);esse de déplacement invalide (0 < Vitesse < ',num2str(vLim),' mm/s)...'],'Erreur ...');
            break;
    end
    %%
...
%         fopen(s);
     if dir
            if (posAct-pas<prLim(1))||(posAct-pas>prLim(2)+40)
...
%         msgbox('deplacement vers le bas ')
        ...
        set(findobj(myinterf, 'Tag','posValue'),'String',['Position : ',num2str(posAct), ' mm']);
            ...
            if pas+posAct>(prLim(2)-10)
                errordlg('Déplacement Hors de la plage...1','Erreur ...');
                break;
            end
